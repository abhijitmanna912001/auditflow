...
export async function fetchWorkpaper(): Promise<MaybeWorkpaper> {
  try {
    const resp = await fetch("http://127.0.0.1:8000/run-case", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      // No payload required for the current backend contract; adjust if needed.
      body: JSON.stringify({}),
    });
    if (!resp.ok) {
...
export const workpaperPayloadFallback: Workpaper = {
  ...
  rows: [
    ...
    { document: "INV-1004", finding: "Amount mismatch", evidence: ["PO-1004", "INV-1004", "REC-1004", "BNK-1004", "LED-1004"], confidence: confidence(0.97), action: "auto_clear" },
  ...
  summary: { items_reviewed: 4, auto_cleared: 2, human_review: 2, critical: 3, assumed_minutes_per_item: 3, estimated_minutes_saved: 6 },
};
